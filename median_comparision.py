...
if __name__ == "__main__":
    ...
    new_df = pd.DataFrame({'GATE_NAME': df1['GATE_NAME'], 'GATE_TYPE': df1['GATE_TYPE'],
                           'MEAN_DIFF': diff_column, 'SIZE_DIFF': diff_column3, 'LIM2': df1['MEAN'],
                           'LIM4': df2['MEAN']}).sort_values(by='MEAN_DIFF',
                                                             ascending=False)

    with pd.option_context('display.max_rows', None, 'display.max_columns', None):  # more options can be specified also
        print(new_df)

    total = len(diff_column)
    sus_amnt = len([a for a in diff_column if a > 1])

    print("\nSuspicious wires (mean_diff>1): " + str(sus_amnt) + "/" + str(total))
